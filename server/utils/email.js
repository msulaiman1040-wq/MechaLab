const sendVerificationEmail = async (email, token) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Brevo failed with status ${response.status}: ${JSON.stringify(data)}`);
    }
  } catch (error) {
...
const sendPasswordResetEmail = async (email, token) => {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
      console.error("Brevo Password Reset Error:", data);
      throw new Error(`Brevo failed with status ${response.status}`);
    }
  } catch (error) {
    console.error("CRITICAL PASSWORD RESET ERROR:", error.message);
